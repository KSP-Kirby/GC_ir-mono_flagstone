    ...
    for dataFactor = [90]                                        % for nice smooth along a single line, 15, 400, 40, 0 works well
        for horzNeighborMaskWeight = [5]
            for verticalNeighborMaskWeight = [2]
                ...
                % use these settings for best accuracy (these are correct,
                % but cause a funny looking visual reconstruction)
                d = 300           % compensates for image scaling, gives much better depth estimate, but poor visual results on right side
                pixelDim = .006;  
                
                % use these settings for best visual reconstruction
                d =  300;           % dual focal length baseline, use this to get better reconstruction
                %pixelDim = .006;
